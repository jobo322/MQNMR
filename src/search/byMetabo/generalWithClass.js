...
let { predictShift } = require(pathToPredictor);
...
module.exports = function (ps, xy, options) {
  ...
  for (let i = 0; i < ps.peaks.length; i++) {
    let signal = ps.peaks[i];
    if (signal.skip) continue;
    intPattern.push(signal.integral);

    let shift = { delta, nH: signal.integral, selected: [], integral: -0.1 };

    //antes se sumaba 1 a cada indice
    let peaks = general.createPeaks(i, options);
    peaks = general.getPeaksInRange(i, peaks);

    let candidates = general.getCandidatesByJ(i, peaks);

    let shiftPredicted = predictShift(toExport, ps.name);

    for (let j = 0; j < candidates.length; j++) {
      delta = utils.getDelta(candidates[j].peaks) + 1;
      if (!shiftPredicted) shiftPredicted = [ delta + 1 ];
      candidates[j].deltaScore =
        (1 - Math.abs(delta - shiftPredicted[0] - 1) / delta) * 10;
    }
